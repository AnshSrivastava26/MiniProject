...
const StudentsEnrolled = () => {
  ...
  const { fetchEnrollmentRecords, fetchStudents } = useContext(AppContext);
  ...
  useEffect(() => {
    loadEnrolledStudents().catch(console.error);
  ...
  return enrolledStudents && students ? (
    <div className="min-h-screen flex flex-col items-start gap-8 md:p-8 md:pb-0 p-4 pt-8 pb-0 ">
      <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-500/20">
        <table className="table-fixed md:table-auto w-full overflow-hidden pb-4">
          ...
          <tbody>
            {students.map((student) => (
              <tr key={student._id} className="border-b border-gray-500/20">
        ...
        {students.length === 0 && (
          <p className="text-gray-500 py-6">No student accounts yet.</p>
        )}
      </div>
      ...
      <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-500/20">
        ...
        {enrolledStudents.length === 0 && (
          <p className="text-gray-500 py-6">No student enrollments yet.</p>
        )}
      </div>
    </div>
  ) : (
