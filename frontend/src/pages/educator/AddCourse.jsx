...
const AddCourse = () => {
  ...
  const { becomeEducator, createCourse, isEducator, navigate } =
  ...
  const addLecture = () => {
    if (
      ...
      setStatusMessage("Lecture title, duration, and URL are required.");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!thumbnailUrl.trim()) {
      setStatusMessage("Please add a course thumbnail URL.");
      return;
    }

    if (!courseDescription || courseDescription === "<p><br></p>") {
      setStatusMessage("Please add a course description.");
      return;
    }
    ...
    try {
      ...
      await createCourse({
        ...
        courseThumbnail: thumbnailUrl.trim(),
        courseContent: chapters.map(({ collapsed, ...chapter }) => chapter),
        isPublished: true,
      });
  ...
  return (
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
      ...
      >
        ...
        {thumbnailUrl && (
          <img
            className="max-h-28 w-max rounded border border-gray-200"
            src={thumbnailUrl}
